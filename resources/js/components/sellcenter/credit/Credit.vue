  ...
  <div>
    ...
    <div class="content-wrapper">
      <section class="content-header">
        <h1>
          <router-link  :to="{ name: 'sell_center_credit_add' }" class="btn btn-primary"
            ><i class="fa fa-plus"></i
          ></router-link>

          <router-link :to="{ name: 'sell_center_debit' }" class="btn btn-info">Debit</router-link>
...
export default {
  ...
  methods: {
    creditList(page = 1) {
      this.loading = true;
      axios
        .get("/api/sellcenter/credits?page=" + page, {
          params: {
            item: this.item,
            status: this.status,
            search: this.search,
    ...
    trash(creditId, index) {
      Swal.fire({
        ...
        if (result.value) {
          axios
            .get("/api/sellcenter/credit/destroy/" + creditId)
            .then((resp) => {
              console.log(resp);
