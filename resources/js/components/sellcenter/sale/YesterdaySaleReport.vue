                ...
                <div class="box-body">
                  <table class="table table-striped text-center table-hover table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr 
                        v-for="(item, index) in yesterday_sales_products"
                        v-else
                        :key="index"
                      >
                        ...
                        <td>{{ (purchasePrice(item.purchase_items) * salesQuantity(item.yesterday_sales)).toFixed(2) }}</td>
                        <td>{{ salesQuantity(item.yesterday_sales) }}</td>
                        <td>{{ salesAmount(item.yesterday_sales) }}</td>
                        <td>{{ (salesAmount(item.yesterday_sales) - ( purchasePrice(item.purchase_items) * salesQuantity(item.yesterday_sales))).toFixed(2) }}  </td>
  ...
  methods: {
    salesList() {
      axios.get("/api/sellcenter/sale/reports/analysis")
        .then((resp) => {
          console.log(resp);
          this.yesterday_sales_products = resp.data.yesterday_sales_products;
