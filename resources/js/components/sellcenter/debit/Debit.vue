  ...
  <div>
    ...
    <div class="content-wrapper">
      <section class="content-header">
        <h1>
          <router-link :to="{ name: 'sell_center_debit_add' }" class="btn btn-primary"
            ><i class="fa fa-plus"></i
          ></router-link>
            <router-link :to="{ name: 'sell_center_credit' }" class="btn btn-info"
            >Credit</router-link>
        </h1>
        <ol class="breadcrumb">
          <li>
...
export default {
  ...
  methods: {
    debitList(page = 1) {
      this.loading = true;
      axios
        .get("/api/sellcenter/debits?page=" + page, {
          params: {
            item: this.item,
            status: this.status,
            search: this.search,
    ...
    trash(debitId, index) {
      Swal.fire({
        ...
        if (result.value) {
          axios
            .get("/api/sellcenter/debit/destroy/" + debitId)
            .then((resp) => {
              console.log(resp);
